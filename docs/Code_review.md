...
| I-05 | `src/models/OcrModel.cpp:92-105`                         | 68         | Parentless `QFutureWatcher` lambda captures `this` → UB if OcrModel dies mid-request (no live path found; shutdown/recipe-switch only). Note: OcrModel is not a QObject — "parent the watcher" is not applicable without a larger refactor          | ASan run with destroyed controller + stalled mock reply                                                      |
...
3. **ASan / verify-only:** I-04/I-05/I-06/I-07 — needs an ASan-instrumented build (I-05 note: `OcrModel` is not a QObject, so "parent the watcher" is not applicable without a larger refactor). Investigations I-01, I-02, I-10…I-15 (§3) — each has a verify recipe; I-15 partially mitigated by the D-Q-15/D-Q-21 changes.
