...
> 2.2, 2.4, 2.5–2.7, все баги 4.1–4.9 и компаратор 1.3. Остаются (см. раздел 5):
> упрощения 1.1 (отдельно), 1.4, 1.5, 2.1, 2.3, 2.8, 3.4–3.8.
...
| 1.4 | Велосипед | `ModelMemoryEstimator.cpp:57-203` | Рукописный парсер GGUF-заголовка (+ UB, см. 4.x) | **FIXED** (см. §1.4) |
| 1.5 | Велосипед | `ModelInstaller.h` | QML-модель списка через `int index → QVariantMap` | OPEN |
| 2.1 | Костыль | `LlamaServerProcess.cpp:306-343` | Хрупкий парсинг прогресса llama.cpp по stdout/stderr | **FIXED** (см. §2.1) |
| 2.2 | Костыль | `ReleaseCatalog.cpp:237-251` | Таймаут сети через `QEventLoop`+`QTimer` | **FIXED** (см. §2.2) |
| 2.3 | Костыль | `LlamaServerProcess.cpp:176-192` | Health-опрос без single-flight | **FIXED** (см. §2.3) |
| 2.4 | Костыль | `LlamaServerProcess.cpp` | TOCTOU-гонка свободного порта | **FIXED** (см. §2.4) |
...
| 2.8 | Костыль | `ModelRegistry.cpp:116` | Сентинел `ctxSize==8192` перегружен как реальное значение | **FIXED** (см. §2.8) |
...
Решение: ридер переведён с ручных смещений/`qFromLittleEndian` на
`QDataStream` поверх ограниченного префикса (8 МиБ) с
`setByteOrder(LittleEndian)`; каждая операция проверяется через `status()`.
`Float32`/`Float64` читаются как сырые little-endian битовые паттерны через
`>> quint32/quint64` и копируются в `float`/`double` через `std::memcpy` —
единый `setFloatingPointPrecision` не может обслужить заголовок со смешанными
Float32 и Float64, а memcpy исключает strict-aliasing UB (п. 4.6, был закрыт
ранее). Строки защищены от злонамеренных длин (не аллоцируются больше, чем
осталось в префиксе). Поведение сохранено: тесты
`test_model_memory_estimator` (включая новый `parsesMixedTypeMeta` — string/
float32/int8/uint64/array вперемешку с нужными Uint32) проходят.
...
именно сместить парсинг (`"5"` внутри постороннего слова, last-%-гонза).
...
Решение: `parseLoadPercent()` переписан на стабильный маркер — регулярное
выражение, берущее **последний** токен `"<число>%"` в строке
(`(\d{1,5}(?:[.,]\d{1,3})?)\s*%`), а не ручной `lastIndexOf` + обратный
проход. Целочисленные и дробные проценты (`25%`/`25.00%`) читаются одинаково;
запятая как десятичный разделитель нормализуется в `toDouble()`. Гейт по
подстроке `"tensors"` сохранён (устойчив к обеим формам llama.cpp:
`llama_model_loader: - loading tensors, N%` и legacy `load_tensors: N%`).
...
Решение: добавлен флаг single-flight `bool m_healthInFlight`. Таймерный тик
пропускает новую пробу, если предыдущая ещё в полёте, и выставляет флаг перед
`GET /health`; `onHealthReply()` снимает его в начале (сигнал `finished`
означает, что запрос завершён — в т.ч. по ошибке/отмене). Плюс сброс в
`spawn()` рядом с `m_healthReached`, чтобы авто-рестарт не унаследовал
застрявший in-flight от прошлой попытки. Поведение на loopback не меняется;
исключено накопление запросов к медленному endpoint. Fallback `/v1/models`
(одноразовый) не трогает этот флаг.

### 2.4 — TOCTOU-гонка свободного порта
**Файл/строки:** `src/runtime/LlamaServerProcess.cpp` (`pickFreePort`, `spawn`,
`resolvedPort`)
**Статус:** FIXED
...
полностью; минимум — ловить `failed to bind` и выкатывать автоматическое
сообщение «Port is busy».

**FIXED (2.4):** часть «ловить failed to bind» уже была реализована
(`RuntimeController::translateServerLine` → «Port is busy»). Закрыт и стык-блокер
в самом автоподбире: `m_opts.port==0` теперь остаётся устойчивым маркером
«auto-pick», а конкретный порт держится в `m_port` и ре-выбирается на каждый
`spawn()` (и первый запуск, и авто-рестарт). Если выбранный порт был занят между
пин-пробой и биндингом дочки, следующий рестарт получает СВЕЖИЙ порт, а не
застревает на уже занятом (бюджет рестартов не тратится впустую). `--port`,
`owner.json` и `resolvedPort()` (`RuntimeController::buildManagedConnection`)
читают `m_port`, поэтому при авто-рестарте соединение корректно переходит на
новый порт. Полный переход на «порт 0 → читать из вывода» сознательно не делался
(крупное изменение/риск).
...
Решение: добавлен `bool ModelEntry::ctxSizeSet` — «ctxSize выбран явно». Теперь
`entryToJson()` пишет `ctxSize` при `ctxSizeSet` (в т.ч. 8192), а
`entryFromJson()` выставляет флаг, когда ключ `ctxSize` присутствует в index.
`ModelInstaller::completeInstall()` помечает ctxSize как явный при
`m_pending.ctxSize > 0`. Дефолт 8192 для не заданного значения сохранён;
покрыто тестом `persistsExplicitDefaultCtxSize`.
