...
#### H.3. Проверка обновлений (opt-in)

1. [ ] Пункт **Check for updates** делает только проверку (без автоустановки):
   GitHub releases + sha256, кэш 6 ч, учёт `runtime/checkUpdates` (opt-in).
2. [ ] UI: плашка «Доступна новая сборка bNNNN (обновлено HH:MM)» в Settings →
   Runtime с кнопками **«Обновить»** / **«Посмотреть изменения»** (ссылка на
   release на GitHub).
3. [ ] Отложенная установка: если сервер `Ready`, предложить «обновление после
   остановки сервера», а не прерывать работу.
...
перезапуска при распознавании, тултипы). Остались **H.3** и опциональные
**H.4–H.5**.
