...
### Stage H — Полировка и документация
...
#### H.2. Оценка памяти и предупреждения (§§4.1, 7.8)

1. [ ] **Оценка VRAM/RAM:** размер GGUF-файла + KV-cache от `ctxSize`
   (`2 × n_layer × n_kv_head × head_dim × ctx × bytes`) → суммарная оценка;
   сравнение с доступной RAM (`QProcessEnvironment`/системным API) и VRAM
   (для CUDA/Metal/Vulkan — приближённо).
2. [ ] **Предупреждение** в шаге **Launch** мастера и в Settings → Launch, если
   оценка превышает доступную память: жёлтая плашка + предложение уменьшить
   `--n-gpu-layers` или `--ctx-size` (по мотивам §7.5 и §5.3).
3. [ ] **Показать оценку** рядом с выбором модели (в шаге Model): «~N GiB
    RAM + ~M GiB VRAM».
...
#### H.7. Полировка процессов и производительности
