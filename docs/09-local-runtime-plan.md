...
#### H.4. Процессы и no-orphan (опционально, §§5.4)
...
1. [ ] **watchdog-helper для macOS**: фоновый helper, следящий за родителем через
   ...
   полная (не best-effort) гарантия no-orphan.
2. [ ] Если helper не выпускается в MVP — задокументировать честную формулировку
   и оставить «обнаружение осиротевшего процесса при следующем старте» как есть.

---

#### H.5. Секреты в системном кейчейне (опционально, §§7.6)
...
2. [ ] UI: пометка «хранится в системном хранилище», скрытое поле, кнопка
   «забыть токен».
3. [ ] Логи, ошибки и `toDisplayCommand()` — по-прежнему без секретов.
...
Runtime). Остались опциональные **H.4–H.5**.
