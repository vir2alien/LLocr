...
1. [ ] **Окно лога** (§8):
   - [ ] кнопки **«Скопировать лог»** (весь кольцевой буфер в буфер обмена)
     и **«Открыть каталог»** (папка `logs/` через `QDesktopServices::openUrl`);
   - [ ] автоскролл вниз при появлении новых строк (кроме состояния, когда
     пользователь сам проскроллил вверх);
   - [ ] индикация «живого» обновления и кнопка очистки поля просмотра.
2. [ ] **Пустые/пограничные состояния**: подсказки в `WorkPanel` и `ThumbPanel`
...
3. [ ] **Полировка подтверждения остановки**: консистентный текст Статуса при
   Stop во время `StartingRuntime` vs `Recognizing` (не «ошибка», а «остановлено»).
4. [ ] **Баннер перезапуска §G-UI**: добавить кнопку «Скрыть» (баннер не должен
   блокировать работу, только напоминать) и перезапуск с подтверждением, если
   идёт распознавание.
5. [ ] Доступность/относящиеся детали: tooltip на точку-индикатор, увеличенная
