...
1. [ ] Сократить `waitForStarted`/health-интервалы под стать быстрым стартам
   (без потери стабильности); проверить отсутствие тормозов UI во время
   `RuntimeLocator::probe` на больших дисках.
2. [ ] Код-ревью вывода лога и статусов; классифицировать прогресс загрузки
   модели из stderr (процент) и показать его в поле UI.
3. [ ] Убрать/свести к минимуму дублирующиеся строки в `llocr_ru.ts`; проверить,
   что нет «vanished» вокруг новых контекстов (см. раздел про переводы).
