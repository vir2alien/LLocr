...
- ✅ Поддержка `.tar.gz` — реализована (llama.cpp публикует `.zip` только для Windows; macOS и Linux идут `.tar.gz`). См. ADR 34 (изменён).
...
├── ArchiveExtractor.h/.cpp     # ZIP + tar.gz, hardened (zlib)
...
- Полноценный watchdog-helper `kqueue`/`NOTE_EXIT` был вынесен в **Stage H**
  как опциональное улучшение — решение: **закрыт, не реализуется** в MVP (см. H.4).
...
1. **`ReleaseCatalog`**
   ...
   - Разбор имён (`llama-b<build>-bin-<os>-<backend>-<arch>.zip` и
     `llama-b<build>-bin-<os>-<arch>.tar.gz`, включая `ubuntu` → `linux`) регуляркой
     + таблица известных шаблонов. Ассеты без backend-токена (macOS, generic
     Ubuntu) считаются универсальными (подходят любому backend-запросу).
     Неизвестные ассеты — только в advanced-режиме.
...
4. **`ArchiveExtractor` — ZIP и `.tar.gz`** (zlib; без приватных Qt-заголовков
   и системного `tar`/`unzip`).
   llama.cpp публикует `.zip` только для Windows; macOS
   (`llama-b<build>-bin-macos-<arch>.tar.gz`) и Linux/Ubuntu (`...-ubuntu-*.tar.gz`)
   идут `.tar.gz` — без TAR-поддержки установка на macOS невозможна (ADR 34 изменён).
   macOS-тарболы содержат symlink-записи для версионированных `.dylib`;
   экстрактор воссоздаёт **относительные** symlink (абсолютные и с `..` — отбрасываются),
   hardlink-записи пропускаются с предупреждением.
   ...
   **Обязательный hardening:**
   ...
   - symlink разрешается только для относительных целей без `..` внутри нашего
     каталога; unsafe-цели пропускаются с предупреждением;
   - лимиты anti-bomb: суммарный размер (≤4 ГБ), количество файлов (≤10 000),
     коэффициент сжатия (≤200:1, ZIP);
...
6. Кнопка «Очистить неиспользуемые сборки» (не трогает активную).
   Защита: пока нет активной сборки (например, сразу после сброса настроек)
   очистка **отклоняется** — иначе без keep-tag она удалила бы все скачанные
   сборки.

7. Переключение между установленными сборками без повторной загрузки
   (ADR 60): `InstallTransaction::scanInstalledBuilds()` перечисляет каталоги
   `<runtimeDir>/llama.cpp-*` (парсит build/backend из тега, находит
   llama-server), `RuntimeInstaller::activateBuild(index)` делает сборку
   активной коммитом настроек (`serverPath` + `installedBuild` + `backend`);
   список «Установленные сборки» с кнопкой Activate — в `Settings → Runtime`
   и в шаге Runtime первого запуска. Сценарий: сброс настроек стирает
   `runtime/serverPath`/`installedBuild`/`backend`, но каталоги сборок
   остаются на диске — активация возвращает сборку одним кликом, без
...
список установленных сборок (Activate).
...
### Stage F — Мастер первого запуска ✅ (выполнен)

**Задачи**

1. [x] Триггер строго по §4.4 (`setupVersion == 0 && !setupDismissed`),
   **без сетевых проверок** (Main.qml Timer).
2. [x] Шаги:
...
3. [x] Навигация: Назад / Далее / Пропустить; шаг не покидается вперёд, пока его
   условие не выполнено. Крестик → `setupDismissed = true`.
4. [x] Все строки через `qsTr`, добавлены в `llocr_ru.ts`.
...
### Stage G-UI — Интеграция в основной интерфейс ✅ (выполнен)

**Задачи**

1. [x] Индикатор в `Footer.qml`: точка (серый `Stopped` / жёлтый `Starting` /
   зелёный `Ready` / красный `Failed`) + текст + клик → `ServerLogWindow`.
2. [x] Баннер «Параметры запуска изменены — требуется перезапуск сервера»
   при правке `launch/*` во время `Ready`, с кнопкой «Перезапустить».
3. [x] Прогресс `StartingRuntime` с текстом из stderr («loading model … 43%`).
4. [x] Ошибки — по матрице §7.5.

---

### Stage H — Полировка и документация 🔄 (в процессе)

Заключительный «полирующий» этап: UX-детали, безопасность/секреты, надёжность
процессов и приведение документации в соответствие с фактическим состоянием.
Код не трогается по крупному — только точечные улучшения поверх завершённых
стадий A–G.

---

#### H.1. Полировка UI/UX ✅ (выполнено)

1. [x] **Окно лога** (§8):
   - [x] кнопки **«Скопировать лог»** (весь кольцевой буфер в буфер обмена)
     и **«Открыть каталог»** (папка `logs/` через `QDesktopServices::openUrl`);
   - [x] автоскролл вниз при появлении новых строк (кроме состояния, когда
     пользователь сам проскроллил вверх);
   - [x] индикация «живого» обновления и кнопка очистки поля просмотра.
2. [x] **Пустые/пограничные состояния**: подсказки в `WorkPanel` и `ThumbPanel`
   при пустом документе; нейтральный текст футера «Рантайм не настроен» -> кнопка
   ведёт в Settings → Runtime/Models (аналог банера G-UI).
3. [x] **Полировка подтверждения остановки**: консистентный текст Статуса при
   Stop во время `StartingRuntime` vs `Recognizing` (не «ошибка», а «остановлено»).
4. [x] **Баннер перезапуска §G-UI**: добавить кнопку «Скрыть» (баннер не должен
   блокировать работу, только напоминать) и перезапуск с подтверждением, если
   идёт распознавание.
5. [x] Доступность/относящиеся детали: tooltip на точку-индикатор, увеличенная
   контрастность при выбранной теме, кликабельность всей строки футера.

**Примечание:** 1–3, 4 («Скрыть»), 5 были реализованы ранее; в этом заходе
добавлено **подтверждение перезапуска при идущем распознавании** (кнопка
Restart в баннере → диалог «Restart server?» при `controller.busy`, иначе
`doRestart()` сразу) + ru-переводы.

---

#### H.2. Оценка памяти и предупреждения (§§4.1, 7.8) ✅ (выполнено)

1. [x] **Оценка VRAM/RAM:** размер GGUF-файла + KV-cache от `ctxSize`
   (`2 × n_layer × n_kv_head × head_dim × ctx × bytes`) → суммарная оценка;
   сравнение с доступной RAM (системный API/`QProcessEnvironment`) и VRAM
   (для CUDA/Metal/Vulkan — приближённо). Реализовано в `ModelMemoryEstimator`
   (`estimateModelMemory()` + `systemPhysicalRamBytes()`), тест
   `test_model_memory_estimator`.
2. [x] **Предупреждение** в шаге **Launch** мастера и в Settings → Launch, если
    оценка превышает доступную память: жёлтая плашка + предложение уменьшить
    `--n-gpu-layers` или `--ctx-size`.
3. [x] **Показать оценку** рядом с выбором модели (в шаге Model): «~N GiB
    RAM + ~M GiB VRAM».

---

#### H.3. Проверка обновлений llama.cpp (opt-in) ✅ (выполнено)

1. [x] Пункт **Check for updates** делает только проверку (без автоустановки):
   GitHub releases + sha256, кэш 6 ч, учёт `runtime/checkUpdates` (opt-in):
   авто-проверка при открытии вкладки Runtime только при включённом opt-in
   (чекбокс «Check for updates automatically» в Settings → Runtime).
2. [x] UI: плашка «Доступна новая сборка bNNNN (обновлено HH:MM)» в Settings →
   Runtime с кнопками **«Обновить»** / **«Посмотреть изменения»** (ссылка на
   release на GitHub).
3. [x] Отложенная установка: если сервер `Ready`, плашка предлагает
   «обновление после остановки сервера» (кнопка «Stop server and update»),
   а не прерывает работу.

---

#### H.4. Процессы и no-orphan (закрыто — решено не реализовывать, §§5.4)

**Решение (после оценки):** watchdog-helper не выпускается в MVP — закрыт без
реализации как неоправданный по соотношению сложность/польза (см. ADR 47).

1. [ ] **watchdog-helper для macOS**: фоновый helper, следящий за родителем через
   `kqueue`/`NOTE_EXIT`, гасящий `llama-server` при аварийном завершении GUI ——
   полная (не best-effort) гарантия no-orphan. *(не делаем)*
2. [x] Если helper не выпускается в MVP — задокументировать честную формулировку
    и оставить «обнаружение осиротевшего процесса при следующем старте» как есть.
    → Честная формулировка зафиксирована в §5.4 и ADR 30; оставлено как есть.

---

#### H.5. Секреты в системном кейчейне (отложено, §§7.6)

**Решение**: откладывается. Остаётся как плановое улучшение (см. ADR 48).

1. [ ] Перенести `provider/apiKey` и `hf/token` в системный кейчейн
   (Keychain на macOS / `Credential Manager` на Windows / `libsecret` на Linux)
   вместо plaintext в `QSettings`; миграция существующих значений при первом
   запуске версии H.
2. [ ] UI: пометка «хранится в системном хранилище», скрытое поле, кнопка
   «забыть токен».
3. [x] Логи, ошибки и `toDisplayCommand()` — по-прежнему без секретов.
   *(это уже действует — см. §7.6)*
...
#### H.7. Полировка процессов и производительности ✅ (выполнено)

1. [x] Сократить `waitForStarted`/health-интервалы под стать быстрым стартам
   (без потери стабильности): `waitForStarted 10s→5s`, health-интервал `500→250ms`;
   в `RuntimeLocator::runProbe` граница запуска процесса `2.5s` + таймаут
   `10s→5s`; `probeCached()` на пути `startServer` (LRU по path/mtime/size)
   устраняет повторный спавн-процесс — нет тормозов UI на больших дисках
   (кэш инвалидируется при изменении файла).
2. [x] Код-ревью вывода лога и статусов: классификация stderr в процентах
   (`loading tensors, NN%` / `load_tensors: NN%`) → `loadProgressPercent()` +
   статус «Loading model… N%»; милстоуны «Preparing context…»; `ProgressBar` в
   футере становится детерминированным, когда прогресс известен.
3. [x] Убраны дубли/«vanished» в `llocr_ru.ts`: починен битый `<message>`
   Footer (External + Runtime: not configured), удалён vanished-контекст
   `llocr::ModelRegistry`, финализированы 3 переведённых `unfinished`-строки.
...
| Файл | Что добавить/исправить |
| --- | --- |
| `docs/01-overview.md` | **Runtime/Models**: сценарий «установить llama.cpp+модель и распознать»; режимы `External`/`Managed`; упоминание мастера первого запуска (Stage F); обновить схему Settings (вкладки Connection/Model/…) и пометку «prompt жёстко зашит» (теперь часть парсера/пресетов). |
| `docs/02-tech-stack.md` | Qt PDF уже есть; добавить про локальный рантайм (llama.cpp CLI, health `/health`), модели с Hugging Face (GGUF), `DownloadTask`/`DownloadManager`, `ProcessGuard`; воспроизводимость — пиннинг коммитов и sha256. |
| `docs/03-architecture.md` | **runtime-слой** на диаграмме слоёв: `RuntimeController` (facade / resolve), `RuntimeInstaller` (D), `ModelInstaller`/`ModelRegistry` (E), `LlamaServerProcess`/`ProcessGuard`, `DownloadTask`/`ArchiveExtractor`/`InstallTransaction`; принцип ARM-координации (`ensureConnectionReady()`, ADR 26/32/37); добавить `ResolvedConnection` в ключевые абстракции. |
| `docs/04-components.md` | новые разделы: **4.16 Local runtime (managed llama.cpp)**, **4.17 Модели и каталог (HF)**, **4.18 Мастер первого запуска (Stage F)**, **4.19 Эскиз UI (G-UI)** с таблицей состояний сервера и матрицей ошибок §7.5; обновить `4.12 Тесты` — новые таргеты и мок-сервер. |
| `docs/05-roadmap.md` | снять стадии A, B, C, D, E, G-core, F, G-UI с «текущего»; отметить H как завершающий; добавить статус по каждому этапу локального рантайм-плана. |
| `docs/06-dev-setup.md` | добавить: локальные рантайм и модели не требуют внешнего Python; описать установку и тесты; временные каталоги (`runtime/`, `models/`, `cache/`, `logs/`) и переменные окружения. |
| `docs/07-glossary.md` | термины: `ResolvedConnection`, `AppBusyState`, `RuntimeState`, `runSelfTest`, alias, `Managed`/`External`; ADR-записи для новых решений (если добавлены новые) — пересмотреть список ADR 26–45. |
| `AGENTS.md` | сократить «Current status» до итога A–G; внести статусы новых стадий; убедиться, что build/test инструкции по-прежнему верны. |

**Приёмка по документации:** каждая страница 01–07 и `AGENTS.md` не содержит
утверждений, противоречащих коду (проверить по тестам и ручному сценарию), а
новая функциональность runtime/model/мастер описана ровно в одном месте.
...
| модель не выбрана (Managed старт) | «Модель не выбрана — выберите модель в Settings → Models или в мастере настройки»; `startServer()` отказывает до запуска (ADR 61) |
| сервер Ready, но настроек конфига нет | Распознавание идёт по работающему серверу: гейт `configValid` не применяется, пока сервер жив (Ready/Starting) — истина это `/v1/models` с fallback на первую модель (ADR 61) |

### 7.6. Секреты
`hf/token` и `provider/apiKey` хранятся в `QSettings` **открытым текстом** —
это существующее и принятое поведение приложения; перенос в системный кейчейн
**отложен** (см. H.5 / ADR 48). Обязательно:
...
| 28 | Установка из GitHub Releases `ggml-org/llama.cpp`, распаковка своим кодом (zlib; ZIP + `.tar.gz`), sha256 из тела релиза | Воспроизводимость, без внешних зависимостей; размер ≠ целостность |
...
| 34 | Поддерживаются ZIP и `.tar.gz`; диспетчеризация по расширению в `ArchiveExtractor::extractArchive` | llama.cpp публикует `.zip` только для Windows; macOS и Linux идут `.tar.gz` — на macOS zip-ассета нет вообще (формат ассетов изменён в 2026). macOS-тарболы содержат symlink-записи `.dylib` — воссоздаются как относительные ссылки |
...
Managed-старт, дедупликация, отмена, таймаут, self-test) — **зелёный**.

✅ **F выполнена (UI).** Мастер первого запуска: `SetupWizard.qml` +
`Setup/Step{Welcome,Runtime,Model,Launch,Done}.qml` (qrc). Триггер по §4.4 в
`Main.qml` (Timer, без сетевых проб) + кнопка «Запустить мастер…» в
Settings → Runtime. Навигация Назад/Далее/Пропустить, шаг блокирует переход
вперёд, пока его условие не выполнено (Welcome: выбор пути; Runtime:
`serverPath`; Model: `launchModelPath`; Launch: успешный self-test); крестик →
`setupDismissed = true`. External-путь ставит `setupVersion = 1` и открывает
вкладку Connection. Launch использует новый QML-мост `runSelfTestQml()`
(свойства `selftestRunning/selftestOk/selftestMessage` + сигнал
`selftestFinished`). Все строки в `llocr_ru.ts` (переведены).

✅ **G-UI выполнена (UI).** `Footer.qml`: индикатор managed-рантайма — цветная
точка (grey Stopped / yellow Starting / green Ready / red Failed) + текст, клик
открывает общий `ServerLogWindow` (создан в `Main.qml`, шарится с Settings);
новое состояние запуска — indeterminate-прогресс + текст из stderr
(«loading model …») из `Runtime.statusMessage`; ошибки Failed — по матрице §7.5.
...
opt-in: плашка «Доступна новая сборка bNNNN» с кнопками «Обновить»/«Посмотреть
изменения», отложенная установка при запущенном сервере; авто-проверка при
открытии вкладки — только при `runtime/checkUpdates`, чекбокс в Settings →
Runtime). **H.4 закрыт** (watchdog-helper не реализуется в MVP, см. ADR 47);
**H.5 отложен** (кейчейн — ADR 48).
